...
const CustomizeProduct = ({productId,variants,productOptions}:{productId:string;variants:products.Variant[];productOptions:products.ProductOption[];}) => {
 ...
 const isVariantInStock=(choices:{[key:string]:string})=>{
    return variants.some((variant)=>{
        ...
        return Object.entries(choices).every(([key,value])=>variantChoices[key]===value) && variant.stock?.inStock && variant.stock?.quantity &&
  ...
  return (
    <div className="flex flex-col gap-6">
                ...
                {option.choices?.map((choice)=>{
                    ...
                    const clickHandler=disabled ? undefined : ()=>handleOptionSelect(option.name!,choice.description!)
                     return option.name==="Color" ? (
                        <li className="w-8 h-8 rounded-full ring-1 ring-gray-300  relative" style={{backgroundColor:choice.value, cursor:disabled? "not-allowd":"pointer"}}
                        ...
                        {disabled && <div className="absolute w-10 h-[2px] rounded-full bg-red-400 rotate-45  top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2"/>}
                        </li>
                     ):
                     (
                        <li className="ring-1 ring-ak text-ak rounded-md  py-1 px-4 text-sm"
                            style={{
        ...
        <ul className="flex items-center gap-3">
            <li className="ring-1 ring-ak text-ak py-1 px-4 text-sm cursor-pointer rounded-md">Small</li>
            <li className="ring-1 ring-ak text-white bg-ak py-1 px-4 text-sm cursor-pointer rounded-md">Medium</li>
            <li className="ring-1 ring-pink-200 text-white bg-pink-200 py-1 px-4 text-sm cursor-not-allowed rounded-md">Large</li>
        </ul> */}
    </div>
  )
}
